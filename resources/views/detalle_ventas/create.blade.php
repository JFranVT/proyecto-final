...
<div class="container">
    ...
    <form action="{{ route('detalle_ventas.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="ID_Venta" required>
                ...
                @foreach($ventas as $venta)
                    <option value="{{ $venta->ID_Venta }}">#{{ $venta->ID_Venta }} - {{ $venta->cliente->Nombre ?? 'Sin cliente' }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" name="Precio_Unitario" min="0" step="0.01" value="0" required>
        </div>
